...
**Consultar llms.txt antes de llamar a Context7**. Las librerías principales son:

- `mkdocs`: Generador principal.
- `mkdocs-material`: Tema y extensiones visuales.
- `pyyaml`: Para validación de configuración.

## Semantic Search (Serena MCP)

Para navegar por documentos extensos o estructuras de carpetas, utiliza **Serena MCP**.

1. **Activación**: Llama a `activate_project(project="locus")` e `initial_instructions()`.
2. **Exploración**: `get_symbols_overview` sobre archivos `.md` ayuda a ver el esquema del documento.
3. **Localización**: `find_symbol` permite ir directo a encabezados específicos.
